...
def create_article(db: Session, article: schemas.ArticleCreate, token: str) -> schemas.Article:

    valid_token = auth.get_auth_token(token=token)
    if valid_token:
        db_storage = db.query(models.Storage).filter(models.Storage.uuid == article.storage_uuid).first()
        if not db_storage:
