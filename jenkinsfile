pipeline {
    ...
    stages {
            ...
            steps {
                script{
                    ...
                    if (env.BRANCH_NAME == 'master') {
                        app = docker.build("registry.gitlab.com/kevmaxsarl/epursa/transaction-api:latest")
                    } else {
                        app = docker.build("registry.gitlab.com/kevmaxsarl/epursa/transaction-api:"+env.BRANCH_NAME)
                    }
                }
            }
        }
        ...
        stage('Deploy') {
            steps {
                script{
                    docker.withRegistry('https://registry.gitlab.com/kevmaxsarl/epursa/transaction-api', 'gitlab_username_password') {
